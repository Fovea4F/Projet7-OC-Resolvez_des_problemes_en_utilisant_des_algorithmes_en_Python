...
def bruteForceKnapSac(maxInvestment, stockList, stockListSelection=[]):
    if elemenstockListts:
        val1, listVal1 = bruteForceKnapSac(maxInvestment, stockList[1:], elementsSelection)
        val = stockList[0]
        if val[1] <= maxInvestment:
            val2, listVal2 = bruteForceKnapSac(maxInvestment - val[1], stockList[1:], elementsSelection + [val])
            if val1 < val2:
