...
actionList = [["Action-1", 20, 5], ["Action-2", 30, 10], ["Action-3", 50, 15],
              ...
              ["Action-14", 14, 1], ["Action-15", 18, 3], ["Action-16", 8, 8], ["Action-17", 4, 12], ["Action-18", 10, 14], ["Action-19", 24, 21], ["Action-20", 114, 18]]
...
for index in range(len(bestCombination[0])):
    value = bestCombination[bestInvestIndex][index][0]
    print(f"{value}")
    # bestInvestActionsList.append((combinations[bestInvestIndex][index][0]))
for index in range(len(combinations[bestInvestIndex][0])):
    bestInvestAmount += bestCombination[bestInvestIndex][index][1]
acquiredAmount = roiCalculation(combinations[bestInvestIndex])
