...
def roiCalculation(combinationArray):
    # calculate Return On Investment amount
    roi = 0
    for index in range(len(combinationArray)):
...
def generateCombinations(actionArray, maxInvest):
    ...
    def backtrack(start, current_combination, invest):
        ...
        for index in range(start, len(actionArray)):
            ...
            backtrack(index + 1, current_combination, maxInvest)  # Recursively explore combinations without the current element
            # print(i+1)
...
acquiredRoiAmount = roiCalculation(bestCombination) - bestInvestAmount
strAcquiredRoiAmount = "%.2f" % acquiredRoiAmount
print(f{:.2f}.format({acquiredRoiAmount}))

print(f"Un investissement dans la liste d'actions {bestInvestActionsList},\
 permet un gain de  {acquiredRoiAmount} € pour une acquisition\
 de %.2f.format{bestInvestAmount} €, soit une rentabilité de {0:.2f}.format{acquiredRoiAmount/bestInvestAmount*100}")
