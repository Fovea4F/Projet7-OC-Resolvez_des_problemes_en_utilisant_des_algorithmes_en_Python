...
# dataFile_sort = dataFile.sort_values(by='value')
# print(dataFile_sort)  # Display the first few rows'''
'''for index, row in dataFile_sort.iterrows():
    valeur = row['value']
    action = row['stockName']
    print(f"{action} : {valeur}")'''
actionList = []
for row in range(len(dataFile)):
    lineTuple = ({dataFile.iloc[row, 0]},{dataFile.iloc[row, 1]}, {dataFile.iloc[2]})
    actionList.append(lineTuple)
print(actionList)
...
for stock in range(len(stockList)):
    valueStockResult = stockList[stock][1] * (stockList[stock][row, 2] / 100)
