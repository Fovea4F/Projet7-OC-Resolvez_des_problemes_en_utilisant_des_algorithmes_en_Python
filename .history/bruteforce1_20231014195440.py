...
for index in range(len(bestCombination)):
    action = bestCombination[index][0]
    bestInvestActionsList.append(action)
    # bestInvestActionsList.append((combinations[bestInvestIndex][index][0]))
for index in range(len(bestCombination)):
    bestInvestAmount += bestCombination[index][1]
acquiredRoiAmount = roiCalculation(bestCombination)# The `
# ` is used to create a line break in the printed
# output. It is used to improve the readability of
# the output by separating different parts of the
# message onto separate lines.
...
print(f"Afin de maximiser les profits, vous devers investir dans la \
    ...
    de {bestInvestAmount} €, afin d'acquérir la somme de {acquiredRoiAmount} €")
