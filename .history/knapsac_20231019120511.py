...
def dynamicKnapSac(maxInvestment, stockList):
    ...
    while weight >= 0 and n >= 0:
        ...
        if matrice[n][weight] == matrice[n-1][weight-currentProcessedStock[1]] + currentProcessedStock[2]:
            selectedStockList.append(currentProcessedStock)
            weight -= currentProcessedStock[1]
...
for stock in range(len(stockList)):
    valueStockResult = stockList[stock][1] * ((stockList[stock][2] / 100) + 1)
    valuedStockList.append((stockList[stock][0], stockList[stock][1], valuedStockList))

print('Algo naïf : ', basicKnapSac(500, valuedStockList))
print('Algo force brute : ', bruteForceKnapSac(500, valuedStockList))
print(f"Algo dynamique : {dynamicKnapSac(500, actionList)}")
