...
def bruteForceKnapSac(stockList, maxInvestment, stockListSelection=[]):
    if stockList:
        val1, listVal1 = bruteForceKnapSac( stockList[1:], maxInvestment, stockListSelection)
        val = stockList[0]
        if val[1] <= maxInvestment:
            val2, listVal2 = bruteForceKnapSac(stockList[1:], maxInvestment - val[1], stockListSelection + [val])
            if val1 < val2:
                return val2, listVal2

        return val1, listVal1
    else:
        reponse_A = sum([i[2] for i in stockListSelection])
        reponse_B = stockListSelection
...
def dynamicKnapSac(maxInvestment, stockList):
    ...
    while limitInvest >= 0 and stockListNumber >= 0:
        ...
        if (matrice[stockListNumber][limitInvest] == matrice[stockListNumber-1][limitInvest-currentProcessedStock[1]] + currentProcessedStock[2]):
...
print(f"Algo force brute : , {bruteForceKnapSac(valuedStockList, 500)}")
end = time.perf_counter()
print(f"Temps écoulé pour bruteForce: {(end - start)*1e6}")
