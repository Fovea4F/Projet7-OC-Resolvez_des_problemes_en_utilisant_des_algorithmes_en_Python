        ...
        for index in range(len(tmpSweepArray)):
            ...
            for rightIndex in range(len(rightArray)):
                ...
                if (valuesPrice <= purchaseThreshold):
                    newCombinationArray.append(valuesArray[len(leftArray) + rightIndex])
                    print(f"Nouvelle combinaison : {new}")
                    newCombinationArray = newCombinationArray + [len(leftArray) + rightIndex]
