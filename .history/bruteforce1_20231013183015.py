...
def generate_combinations(actionArray, maxInvest=0):
    def backtrack(start, current_combination):
        # print(f"start: {start}, current_combination : {current_combination}")
        if combinations.append(list(current_combination))  # Add the current combination to the list
        # print(combinations)
...
combinations = generate_combinations(actionList, maxInvest)
